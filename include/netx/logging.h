...
//队列类型(是否启用了MPMC，无锁多生产者多消费者环形队列)
enum class LogQueueType {kBlocking=0,kMPMC};

//日志配置(可在程序启动阶段调用 InitLogging 配置)
struct LoggingConfig{
    bool async = true;                              //是否异步日志（false 则前端直接写）
    LogQueueType queue_type = LogQueueType::kMPMC;  //队列类型，MPMC或简单阻塞队列
    std::size_t queue_capacity = 1024;              //队列容量(MPMC时要求为2的幂)
    const char* log_file = nullptr;                 //日志文件路径，nullptr或空串则输出到stderr
...
//轻量级 LogStream ,支持常用类型的 operator<<,内部使用FixedBuffer
class LogStream{
    public:
        LogStream();
...
//Logger:RAII日志前端，析构时将格式化好的日执行推送到后台
class Logger{
    public:
