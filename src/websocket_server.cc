...
namespace netx
{
    ...
    //和 HttpServer 完全一样的套路：构造时把自己的回调注册到 server_ 上
    WebSocketServer::WebSocketServer(EventLoop *loop, const InetAddress &addr,
                                     int io_threads)
        : base_loop_(loop), server_(loop, addr, io_threads, /*reuse_port=*/false)
    {
    ...
    void WebSocketServer::Start() { server_.Start(); }//Start() 直接转调 server_.Start()
    ...
    void WebSocketServer::OnConnection(const ConnectionPtr &conn)
    {
        if (conn->IsConnected())
        {
            ...
            LOG_INFO << "WebSocket client connected fd=" << conn->fd();
        }
        else
        {
            LOG_INFO << "WebSocket client closed fd=" << conn->fd();
            RemoveConnection(conn);//已经断了，清理残留数据
        }
    }

    void WebSocketServer::OnMessage(const ConnectionPtr &conn, Buffer *buf)
    {
        auto *sess = conn->get_context<Session>();
        if (!sess)
        {
            Session tmp;
            tmp.last_heartbeat = std::chrono::steady_clock::now();
            conn->set_context(std::move(tmp));
            sess = conn->get_context<Session>();
        }
        if (sess->state == Session::State::kHandshaking)
        {
            if (!HandleHandshake(conn, sess, buf))//还没握手，走握手
            {
                return;
            }
        }
        ...
        {
            HandleFrames(conn, sess, buf);
        }
    }

    bool WebSocketServer::HandleHandshake(const ConnectionPtr &conn, Session *sess,
                                          Buffer *buf)
    {
        HttpParser parser;
        HttpRequest req;
        ...
        {   
            //握手必须用 GET 请求，用 POST 或其他方法直接拒绝
            conn->Send(std::string("HTTP/1.1 405 Method Not Allowed\r\n\r\n"));
            conn->Shutdown();
            return false;
        }
        ...
        //buf里的头部信息必须满足 WebSocket 握手要求，否则直接拒绝连接：
        ...
        sess->last_heartbeat = std::chrono::steady_clock::now();
        if (sess->user_id.empty())//没登录就给个默认 ID
        {
            sess->user_id = "web-" + std::to_string(conn->fd());
        }
        SendJsonMessage(conn, "ready");//给客户端发 {"type":"ready"}，告诉它可以开始发命令了
    ...
    void WebSocketServer::HandleFrames(const ConnectionPtr &conn, Session *sess,
                                       Buffer *buf)
    {
        while (true)
        {
            ...
            uint8_t opcode = 0;
            std::string payload;
            if (!ParseFrame(buf, &fin, &opcode, &payload))
            {
                break;
            }
            switch (opcode)//帧的分类
            {
            case 0x1: // 为text  
                HandleTextFrame(conn, sess, payload);//则解析 JSON 命令
                break;
            case 0x8: // close  
                HandleCloseFrame(conn, sess, payload);//断开连接
                return;
            case 0x9: // ping  
                HandlePingFrame(conn, payload);//回pong
                break;
            case 0xA: // pong 
            ...
            default:
                LOG_WARN << "Unsupported WebSocket opcode=" << static_cast<int>(opcode)
                         << " fd=" << conn->fd();
                SendClose(conn, 1003, "unsupported opcode");
                return;
            }
            if (!fin)
            {
                LOG_WARN << "Fragmented frames not supported fd=" << conn->fd();
                SendClose(conn, 1003, "fragment not supported");
                return;
            }
        }
    }
    ...
    //解析帧
    bool WebSocketServer::ParseFrame(Buffer *buf, bool *fin, uint8_t *opcode,
                                     std::string *payload)
    {
        ...
        size_t pos = 0;
        uint8_t b1 = data[pos++];// 第1字节：FIN + opcode FIN = 这是不是完整消息的最后一帧1=是/ opcode = 这帧的类型：0x1是文本，0x8是关闭，0x9是ping
        uint8_t b2 = data[pos++];// 第2字节：MASK(数据有没有被加密（客户端→服务器必须加密，服务器→客户端不加密）) + payload长度
        bool masked = (b2 & 0x80) != 0;// 最高位：有没有 mask

        /*低7位 = 数据长度，但只有7位最大127，所以用特殊值 126/127 表示"长度在后面"*/
        uint64_t payload_len = (b2 & 0x7F);// 低7位是长度
        if (payload_len == 126) // 126 → 后面2字节才是真长度
        {
            if (len < pos + 2)
                return false;
    ...
    void WebSocketServer::HandleTextFrame(const ConnectionPtr &conn, Session *sess,
                                          std::string_view payload)
    {
        ...
        {
            SendJsonMessage(conn, "error", "\"message\":\"bad payload\"");
            return;
        }
        if (cmd.action == "login")
        {
            ...
            {
                SendJsonMessage(conn, "error", "\"message\":\"missing user_id\"");
                return;
            }
            sess->user_id = cmd.user_id;
            SendJsonMessage(conn, "login_ack",
                            "\"user_id\":\"" + EscapeJson(sess->user_id) + "\"");
        }
        else if (cmd.action == "subscribe")
        {
            UpdateSubscriptions(conn, sess, cmd.lights);
            SendJsonMessage(conn, "subscribe_ack",
                            "\"count\":" + std::to_string(sess->subscribed.size()));
        }
        else if (cmd.action == "ping")
        {
            SendJsonMessage(conn, "pong");
        }
        else
        {
            SendJsonMessage(conn, "error", "\"message\":\"unknown action\"");
        }
    }

    void WebSocketServer::HandlePingFrame(const ConnectionPtr &conn,
                                          std::string_view payload)
    {
        SendPong(conn, payload);
    }

    void WebSocketServer::HandleCloseFrame(const ConnectionPtr &conn, Session *sess,
                                           std::string_view /*payload*/)
    {
    ...
    void WebSocketServer::SendText(const ConnectionPtr &conn,
                                   std::string_view payload)
    {
    ...
    void WebSocketServer::SendJsonMessage(const ConnectionPtr &conn,
    ...
    void WebSocketServer::SendClose(const ConnectionPtr &conn, uint16_t code,
    ...
    void WebSocketServer::SendPong(const ConnectionPtr &conn,
                                   std::string_view payload)
    {
    ...
    //连接断开时清理
    void WebSocketServer::RemoveConnection(const ConnectionPtr &conn)
    {
        ...
        for (const auto& id : sess->subscribed)//// 遍历这个连接订阅的所有灯
        {
            ...
            vec.erase(std::remove_if(vec.begin(), vec.end(),
                                     ...
                                     {
                                         auto sp = wp.lock();
                                         return !sp || sp.get() == conn.get();//一种是找到目标了get返回shared_ptr 里的裸指针地址，另一种是发现垃圾顺手清理。
                                     }),
                      vec.end());
            if (vec.empty())
                subs_.erase(it);
