    ...
    *out_ms = static_cast<int>(ms.count());
    ...
    class BlockingQueue {
      public:
        ...
        void stop(){
            ...
            cv_.notify_all();//通知所有人干完手头活下班，别等了
        }
      private:
        std::mutex mutex_;
    ...
    class MpmcQueue{
      public:
        ...
        //store(value, memory_order) store 是 std::atomic 的原子写入方法
        //value要写入的值  memory_order内存序，控制多线程间的可见性
        //memory_order_relaxed =
        //最宽松的序，只保证这个操作本身是原子的，不保证其他变量的顺序。适合计数器这种独立变量
        ...
        bool push(std::string && data) {
            ...
            for(;;){
                ...
                if(diff ==0){
                    /*  原子操作：如果 enqueue_pos_ 还是 pos，就改成 pos+1
                        成功 → 我认领了这个槽位，跳出循环
                        失败 → 别的生产者抢先了，pos 自动更新为最新值，重试*/
      ...
      //序列号的作用 判断这个槽位能不能读/能不能写：
        ...
        const std::size_t buffer_mask_;//buffer_mask_ 就是容量减 1，配合 & 实现快速取模
        std::unique_ptr<Cell[]> buffer_;
        std::atomic<std::size_t> enqueue_pos_;// 生产者下一个要写入的位置
        std::atomic<std::size_t> dequeue_pos_;// 消费者下一个要读取的位置
